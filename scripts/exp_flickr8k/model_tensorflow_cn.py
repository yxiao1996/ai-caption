...
class Caption_Generator():
    ...
    def build_model(self):
        ...
        # sentence = tf.placeholder("int32", [None, self.n_lstm_steps])
        mask = tf.placeholder("float32", [self.batch_size, self.n_lstm_steps])
        # mask = tf.placeholder("float32", [None, self.n_lstm_steps])
...
def preProBuildWordVocab(sentence_iterator, word_count_threshold=30): # borrowed this function from NeuralTalk
    ...
    word_counts = {}
    nsents = 0
    print sentence_iterator[1].decode('utf-8')[0:1].encode('utf-8')
    # print len(sentence_iterator)
    nan_table = pd.isnull(sentence_iterator)
    for sent_num in range(len(sentence_iterator)):      
      sent = sentence_iterator[sent_num]
      if  nan_table[sent_num] == True:
        continue
      nsents += 1
      print sent
      # m = raw_input()
...
#############################
###### Parameters #####
annotation_path = './data/flickr8k-cn/annotations.pickle'
feat_path = './data/flickr8k-cn/flickr8k/'
model_path = './model/flickr8k-cn'
#############################
...
def train(pretrained_model_path=pretrained_model_path): # 
    ...
    # caption_nan_table = pd.isnull(annotation_data['caption'].values)
    # print captions[1]
    ...
    # instead of read feature from .npy, read data from bigfile
    Data_file = BigFile('./data/flickr8k-cn/flickr8k/')

    maxlen = np.max( map(lambda x: len(x) if (pd.isnull(x)==False) else 0, captions) )
    ...
    # instead of images_id, using image_name to extract feature
    image_name = annotation_data['image_name'].values
    ...
    for epoch in range(n_epochs):
        for start, end in zip( \
            ...
            # read features from bigfile
            _, current_feats =Data_file.read(image_name[start:end])
            # pad zero when feature extract nan
            # print len(current_feats)
            while(len(current_feats) < 80):
              current_feats.append(np.zeros(1024))
            # print len(current_feats)
            # transfer python list to numpy array to feed network
            a = np.asarray(current_feats)
            # little twist
            current_feats = a.reshape(-1, ctx_shape[1], ctx_shape[0]).swapaxes(1,2)
            ...
            current_caption_ind = map(lambda cap: [wordtoix[word] for word in cap[:-1] if word in wordtoix], current_captions) 
            ...
            print "Current Cost: ", loss_value, epoch
        saver.save(sess, os.path.join(model_path, 'model'), global_step=epoch)

def test(test_feat='./feats_borg.npy', model_path='./model/model-4', maxlen=20):
    annotation_data = pd.read_pickle(annotation_path)
    captions = annotation_data['caption'].values
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)
    n_words = len(wordtoix)
...
if __name__ == '__main__':
    train()
    # bigfile = BigFile('./data/flickr8k-cn/flickr8k/')
    # print bigfile.names
    # imset = str.split('99679241_adc853a5c0')
